...
export default function DatepickerComponent() {
    ...
    const [selectedDatesLocal, setSelectedDatesLocal] = useState<Set<string>>(selectedDates);
    ...
    function navigateMonth(direction: 'previous' | 'next') {
        setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + (direction === 'previous' ? -1 : 1), 1));
    }
    ...
    const handleDateClick = (date: Date) => {
        setSelectedDatesLocal((prevSelectedDates: Set<string>) => {
            ...
            <div className="datepicker-header d-flex align-items-center justify-content-center mt-4 mb-4">
                ...
                <div className={"col-3"}>
                    <div className="datepicker-navigation-menu d-flex text-end">
                        <span className={"datepicker-navigation"} onClick={() => navigateMonth('previous')}>←</span>
                        <span className={"datepicker-navigation"} onClick={() => navigateMonth('next')}>→</span>
                    </div>
                </div>
            </div>

            <div className={"datepicker-body pb-4 d-flex align-items-center flex-column"}>
                <div className={"datepicker-grid"}>
                    ...
                    <ul className={"datepicker-dates"}>
                        {calendarDays.map((day, index) => (
                            <li 
                                key={index} 
                                className={`datepicker-date ${day.isCurrentMonth ? 'current-month' : 'inactive'} ${day.isToday ? 'active' : ''} ${selectedDatesLocal.has(day.date.toISOString()) ? 'selected' : ''}`} 
